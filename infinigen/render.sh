...
def generate_camera_position(radius, previous_positions, min_angle):
    ...
    while attempts < max_attempts:
        ...
        if complex:
            ...
            delta_theta = math.pi / 3.2  # Adjust as needed
            delta_phi = math.pi / 3.2    # Adjust as needed
...
for i in range(10):
    ...
    base_radius = 10  # Fixed distance from the object
    camera_radius = base_radius * 0.45  

    camera_position, new_vector = generate_camera_position(camera_radius, previous_camera_vectors, min_angle)
    if camera_position is None:
        break  # Exit if no acceptable position is found

    bpy.context.scene.camera.location = camera_position
    look_at(bpy.context.scene.camera, mathutils.Vector((0, 0, 0)))  # Point camera at origin
    ...
    if light_setup in ('4_lights', '6_lights', '8_lights', '16_lights'):
        ...
        for pos in light_positions:
            ...
            max_energy = 1400  # Set max brightness
            min_energy = max_energy * 0.20  # 20% of max brightness
            if random_dim:
        ...
        key_theta = math.radians(60)  # Angle from the Z-axis
        key_phi = math.radians(30)    # Angle in the XY-plane
    ...
    factory = os.path.basename(factory_name)
    print(factory)
    output_file = os.path.join(output_dir, factory, object_name, f"{i:03d}.png")
...
for blend_file in "$blend_dir"/*.blend; do
    ...
    blender_args=("/home/vaclav_knapp/blender-3.6.19-linux-x64/blender" "-b" "$blend_file" "--python" "$python_script" "--" "$object_name" "$output_dir" "$bg_dir" "$factory_name")
