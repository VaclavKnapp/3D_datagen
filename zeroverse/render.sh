...
def generate_camera_position(radius, previous_positions, min_angle):
    ...
    while attempts < max_attempts:
        ...
        if complex:
            theta0 = math.pi / 2  
            phi0 = math.pi        
            delta_theta = math.pi / 2.85  
            delta_phi = math.pi / 2.85    
...
for i in range(10):
    ...
    base_radius = 10 
    camera_radius = base_radius * 0.96  
...
for category_dir in "$blend_dir"/*; do

    if [ -d "$category_dir" ]; then

        for example_subdir in "$category_dir"/*; do

            if [ -d "$example_subdir" ]; then

                shopt -s nullglob
                blend_files=( "$example_subdir"/*.blend )
                shopt -u nullglob
                

                if [ ${#blend_files[@]} -eq 0 ]; then
                    echo "No .blend files found in $example_subdir"
                    continue
                fi


                relative_subdir_path=$(realpath --relative-to="$blend_dir" "$example_subdir")
                target_subdir="$output_dir/$relative_subdir_path"
                ...
                for blend_file in "${blend_files[@]}"; do
                    ...
                    echo "Running blender on $blend_file..."
                    "${blender_args[@]}"
                done
            fi
        done
